...
class RegionLayer(nn.Module):
    def __init__(self, in_channels, grid = (8,8)):
        ...
        self.grid = grid
        #self.convs = nn.ModuleList([nn.Conv2d(in_channels=in_channels,out_channels=in_channels,kernel_size=3,stride=1,padding=1) for _ in range(grid[0]*grid[1])])
        #self.relus = nn.ModuleList([nn.ReLU() for _ in range(grid[0]*grid[1])])
        #self.bns = nn.ModuleList([nn.BatchNorm2d(num_features=in_channels) for _ in range(grid[0]*grid[1])])
